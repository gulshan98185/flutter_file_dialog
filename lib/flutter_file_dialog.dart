...
class FlutterFileDialog {
  ...
  /// Displays a dialog for selecting a location where to save the folder and
  /// saves the folder to the selected location.
  ///
  /// Returns path of the saved folder or null if operation was cancelled.
  /// Throws exception on error.
  static Future<String?> saveFolder(String sourceFolderPath) {
    return _channel.invokeMethod('saveFolder', SaveFolderDialogParams(sourceFolderPath).toJson());
  }
}
...
class SaveFileDialogParams {
  ...
  const SaveFileDialogParams({
    ...
    this.mimeTypesFilter,
    this.localOnly = false,
  })  : assert(sourceFilePath == null || data == null, 'sourceFilePath or data should be null'),
        assert(sourceFilePath != null || data != null, 'Missing sourceFilePath or data'),
        assert(data == null || (fileName != null && fileName != ''), 'Missing fileName');
...
class SaveFolderDialogParams {
  ...
  /// Create parameters for the [saveFile] method.
  const SaveFolderDialogParams(this.sourceFolderPath);
